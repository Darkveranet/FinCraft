...
- [x] Add **Expense Management** screen — `js/pages/treasury/expenses.js` (request/approve/reject/pay
      lifecycle, inline expandable detail rows, wraps Phase 7's `expenses.js`)
- [x] Add **Borrowings** screen — `js/pages/treasury/borrowings.js` (create + per-borrowing
      drawdown/schedule with per-installment accrue/pay-interest/repay-principal, wraps Phase 8)
- [x] Add **Daily Reconciliation** screen — `js/pages/treasury/reconciliation.js` (start/submit-
      count/approve, danger-confirmed variance posting, wraps Phase 10)
- [x] **[NAV FIX] Add treasury screens to the sidebar** — the sidebar renders from `NAV_GROUPS` in
      `js/ui/shell.js`, a list **separate from** the router's `PAGE_REGISTRY`. Registering a route
      in `router.js` makes it reachable by URL hash but does NOT surface it in the nav. `NAV_GROUPS`
      had no treasury entries at all, so all 8 treasury screens were unreachable from the sidebar
      (URL-hash only) until this fix. Added a single "Treasury" nav group after "Finance" listing
      all 8 route keys; labels/icons come from each route def, and `_isNavVisible()` gating is
      unchanged.
...
- **Phase 11 (COMPLETE) — final 3 screens + navigation fix.** Built the last three screens,
  finishing Phase 11 (8 of 8):
  - `js/pages/treasury/expenses.js` — request/approve/reject/pay lifecycle over Phase 7's
    `expenses.js`; status-aware list with inline Approve (confirm) / Reject (reason input) / Pay
    (TELLER_CASH or BANK form) via the same expandable-detail-row pattern as `teller-console.js`.
  - `js/pages/treasury/borrowings.js` — create form + per-borrowing action list over Phase 8;
    PENDING → inline drawdown; ACTIVE → expandable schedule with per-installment Accrue / Pay
    Interest / Repay Principal (each confirming the Dr/Cr legs); CLOSED → read-only. Post-action
    refresh via a dedicated `renderScheduleInto()` (re-renders in place, no collapse-reopen flicker).
  - `js/pages/treasury/reconciliation.js` — start/submit-count/approve over Phase 10; OPEN → inline
    count; SUBMITTED(≠0) → `danger`-confirmed Approve that posts the shortage/overage JE;
    zero-variance auto-approves in the service (no pointless button); variance shown as an explicit
    Shortage/Overage label.
  - **Shared-helper discipline:** extended `shared.js` with `glOptionsHtml` (extracted from
    `settings.js`, which was refactored to consume it) and `statusBadgeClass` (one status→`.badge`
    mapper for all treasury workflows). All three screens reuse the existing office/teller-cashier
    pickers, `fmtMoney`, and the shared `TreasuryReconciliationGapError` "action required" toast.
  - **[NAVIGATION FIX — the reason these screens were "invisible"]** The sidebar is rendered from
    `NAV_GROUPS` in `js/ui/shell.js`, which is a **separate list from the router's
    `PAGE_REGISTRY`**. Routes registered in `router.js` are reachable by URL hash but do NOT appear
    in the sidebar unless their key is also listed in `NAV_GROUPS` — and `NAV_GROUPS` had **no
    treasury entries whatsoever**, so *all 8* treasury screens (not just the 3 newest) had been
    reachable only by typing the hash directly. Fixed by adding one group after "Finance":
    ...
    Each item's label/icon comes from its existing `PAGE_REGISTRY` def (no renaming), and the
    render loop's existing `_isNavVisible()`/`PAGE_REGISTRY[key]` filters are unchanged, so
    permission gating behaves exactly as for every other nav item. **Caveat for a live tenant:**
    every treasury route is gated on `READ_JOURNALENTRY`; if the signed-in user's `perms` array is
    non-empty and lacks that code, the whole Treasury group is hidden by design. With an empty
    perms array (server returned none) everything shows — the common dev/demo case.
  - **Verification:** `node --check` clean on all new/modified files; nav-key ↔ router-key ↔
    index-view alignment cross-checked (all 8 line up); full suite **14 passed / 1 pre-existing
    unrelated failure, unchanged** (the 3 screens wrap already-tested Phase 7/8/10 services). Same
    `jsdom`/`module-integrity.test.js` gap as every prior UI screen (§16) — not re-attempted.
